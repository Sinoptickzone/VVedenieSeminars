...
if (ValidateLines(array)) Console.WriteLine($"Точка пересечения: [{String.Join("; ", Intersection(array))}]");
...
double[] Intersection(string[] arr)
{
    ...
    if (newarr[0] == -0) newarr[0] = 0;
    if (newarr[1] == -0) newarr[1] = 0;
    
    return (newarr);
}

bool ValidateLines(string[] arr)
{
    if (double.Parse(arr[0]) == double.Parse(arr[2]))
    {
        if (double.Parse(arr[3]) == double.Parse(arr[1]))
        {
